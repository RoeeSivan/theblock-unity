...
>    menu at `timeScale 0`).
> 4. Drive west to the beach. **Look at the sand first, before touching the water** - it should read
>    as gold dry sand with a wet tide line at the water's edge, over a gentle rise. Screenshot it.
...
>
> **Two things are still owed and both are the user's call:**
>
> - ⚠ **The "black strip" has NOT been confirmed as the thing that was fixed.** The measured bug is
>   real and is fixed (dry sand rendered 54% wet everywhere; the config's gold was never drawn on any
>   pixel - memory `flat-dry-sand-renders-half-wet`). But the beach was correctly wired the whole time,
>   so a screenshot is still needed to be sure that dull sand is what the user was looking at. **The
>   other candidate is the water itself:** `surface.deepColor` is `0x05354F`, near-black, and the
>   fresnel bottoms out at 0.04 looking straight down - so the sea just off the sand is genuinely very
>   dark from a shallow camera angle. If the strip survives the rebuild, that is what it is, and the
>   fix is `_ShallowColor` reaching further out rather than anything on the beach.
...
| U36 | The sea is a place, not a wall - sand that reads as sand, and GTA-style water entry | **wip - built, awaiting the user's play-test** | `3a357b3` | **USER ASK 2026-08-18, three parts in one message:** *"can we do some effect of sand near the beach? currently we have this black strip. moreover when driving into the water it should not ignite the police... lets do a similar behvaior as we have in real GTA game."* **① THE SAND.** The beach was never missing - shader, material, mesh and the config's gold `dryColor` were all correctly wired since U12. It could not DRAW that gold: `SeaGeometry.SeabedHeight` clamps the seabed to 0 landward, sea level is 0, so `depth` is exactly 0 across every metre of dry sand and `wet = smoothstep(-0.6, 0.5, 0) = 0.54` - the whole beach renders 54% into the dark `wetColor`, with no tide line anywhere. Faithful to `sea.ts`, which has the same bug. Fixed with a **berm**: `SeaGeometry.BermHeight = 1.2 m`, a half sine pinned to 0 at the waterline AND at the city edge (anything that only ramps up leaves a step at one end or a cliff at the other), mirrored into `Beach.shader` via new `_BermRun`/`_BermHeight`. ~67% of the sand now reads fully dry gold with a ~4 m tide line at the water. Memory `flat-dry-sand-renders-half-wet`. ⚠ **NOT yet confirmed to be the user's "black strip"** - a screenshot is still owed; the other candidate is the water's own `_DeepColor` (`0x05354F`, near-black) seen at a steep angle just off the sand. **② THE POLICE.** Never a water rule. Two invisible 8 m walls stood on the waterline at Unity x 430 - `BuildShoreWall` (600 m) and `BuildWorldEdges`'s Seaward side (1400 m) - so aiming a car at the sea was ramming a wall; `CrashSensor` read the closing speed and `CrimeWatch` compared it to `CrashCrimeSpeed = 6 m/s`, so anything over 21.6 km/h was a star. Both walls moved out to the plate's rim (x 700), opening 270 m of drivable sea, and the sea wall now seats on the seabed rather than on y 0 so a sinking car cannot slide out under it. Memory `an-invisible-wall-reads-as-a-game-rule`. **③ THE WATER.** New `Assets/Scripts/Vehicle/WaterEntry.cs`, attached at runtime by both controllers' `Bind` (never a prefab field - `CrashSensor` was attached to nothing for 14 units that way). Four stages: **Wading** (shallow = drag only, reverse out, no stall - a game where the first centimetre is fatal makes the beach a no-go zone), **Flooding** (sustained depth over `drownAt` for 1.6 s / 0.35 s on a bike cuts the engine, buoyancy bleeds to zero over 7 s), **Sunk** (rests on the bottom, `IEnterable.Respawn` retires it home after 6 s). Floats on `SeaSurface.Height`, never `sea.Level` (memory `vertex-displaced-water-has-no-cpu-height`), four-point buoyancy so a nose-first entry rights itself. A bike ejects its rider the instant it drowns; a car ejects at 80% submersion via the existing `VehicleEnterExit.LeaveVehicleNow`. `Drowned` is its own flag on both controllers, NOT a second writer on `EngineDead` (`VehicleDamage` owns that one). Water drag is a FORCE, not Rigidbody damping - damping is stored state and a mid-Play recompile would cache the water value as the original. `GameAudio` stops the engine loop, `FollowCamera` is held 0.6 m above the surface (`TheBlock/Water` is single-sided, so a camera under it sees straight through to the void the plate was cut away for). **Police, per the user's three answers:** water is never a crime (`CrimeWatch` skips any impact whose sensor is in water - the seabed is a real MeshCollider and landing on it at 60 km/h is a textbook head-on contact), and 4 s in the water WIPES the wanted level (`PoliceTuning.WaterEscapeSeconds`), which is GTA's own rule and the honest one here since nothing the police own can follow you onto water. The water condition folds into `CrimeWatch`'s existing `Heat.Frozen` assignment rather than being written from `WaterEntry` - one flag, one owner. **Compiles clean, both assemblies** (headless, via memory `headless-compile-check-via-unity-dotnet`). **NOT done:** no splash VFX or entry sound - held deliberately so the play-test judges one thing; no drowning (the user chose forced bail-out, and there is no player health system to hang an oxygen bar on) |
