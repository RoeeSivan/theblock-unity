...
>
> ### 🚩 NEXT ACTION: **the user play-tests U35h** (recipe in its section below), then **U30a - the macOS build.**
>
> ---
>
...
### U35h, 2026-08-17 - breakable street props - BUILT, awaiting the user's play-test
...
zero props exist and every pole is rigid). **82 props** stand at the kerb - **66 bins** (one per lit
intersection, 1.2 m onto the pavement beside the junction's first pole), **8 cones** (rows of four
in front of the gas station and the auto shop) and **8 benches** (two each at the 7-Eleven, the pizza
place, the gas station and the auto shop, facing the road). A car sends them flying with a light
...
0° tilt** after 6 s of play - the props are dynamic Rigidbodies that are `Sleep()`ed in `Awake`
...
(they reference Compressed material clones); the layout is a serialized list of 82 rows on the
...
clatter, **no star, no dent**. ③ 7-Eleven: bench; a fast hit slides it. ④ on foot, walk into a
cone: it goes over. ⑤ `Settings → Gameplay → Street Props: Off`: everything vanishes, poles rigid.
