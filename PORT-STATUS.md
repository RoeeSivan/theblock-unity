...
| U35f | Side jobs - taxi and deliveries in free roam | **backlog** | | Weakest on the "web could not" test - it is design, not engine - but the strongest on the game: the wallet today has four mission payouts as its only source and the 7-Eleven as its only sink, so free roam earns nothing. A hail-a-fare loop: a pedestrian on the pavement with a marker (they are NavMesh agents already, `CrowdSpawner`), stop beside them, they get in (the `VehicleEnterExit` seat rig, second seat), a destination pin from the POI table, a timer, a payout scaled by A\* distance (`RoutePlanner` gives the number, so the fare is honest), tip if no crash on the way (`CrashSensor`). Deliveries reuse `DeliveryMission`'s pizza flow with a random shop→door pair. Ships behind a `Settings → Gameplay → Side Jobs` toggle; whether it defaults on is argued at build time. Blender: a taxi roof sign, optional. Reuses: `CrowdSpawner`, `Pedestrian`, `VehicleEnterExit`, `MapPois`, `Payouts`, `Wallet`, `RoutePlanner` |
| U35g | Garage - buy and paint cars | **backlog** | | Money sink number two, and a showcase of the config's own paint plumbing: `CarPaint` writes `baseColorFactor` (memory `gltfast-basecolorfactor-gamma`), so a colour picker is a UI over a system that exists. A garage interior (the `Interior` teleport pattern from U13) with the four cars on turntables lit by U29's three-light preview rig, buy with `Wallet`, pick a paint, and the choice persists in `PlayerPrefs` beside `Progress`. Blender: rims / a spoiler as optional bolt-ons, only if U35b's split export already exists (same file, same origins). Ships as a place in the world - present or not by build. Reuses: `Interior`, `CarPaint`, `CarSpawner`, `Wallet`, `Progress`, the U29 studio rig |
| U35h | Breakable street props | **backlog** | | Bins, bus-stop shelters, bollards, café chairs, market stalls: static until struck, then rigidbodies - the exact `LotCar` mechanism from U34 (a static collider still receives the callback), generalised to a `Breakable` component. **Blender:** split each prop into 2-5 pieces with sane origins, < 500 tris each, one shared 1024² atlas; the Sketchfab props' hidden `Collider` node (memory `sketchfab-collider-proxy-node`) must be stripped in the same pass. Ships as world objects; the number placed is the perf knob (start at 40, measure). Cheap, adds life, and the wanted level should NOT count it - a bin is not a crime |
...
- **2026-08-16** (U35) - **The showcase list is chosen: eight additions, five scheduled, three
  ...
  **U35c** police heli at 3★ + GPS route · **U35d** weather · **U35e** stunt jumps + skids +
