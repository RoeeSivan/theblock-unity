...
> `U37a`/`U37b` halves ✅ confirmed 2026-08-19), then a real crowd (**U38** ✅ 2026-08-19).
...
> which lives in the same twenty metres of pavement the user just walked.
>
> **⚠ One small thing on U39 is still unwatched, and it is not the falafel:** the parked cars now stop
> being drawn at **350 m** instead of ~710-950 m. It buys nothing measurable (see the U39 section), and
> the fog does not meaningfully hide anything at that range, so a car winking out in the distance is
> the one plausible regression. If it is ever seen: raise `CullDistance` in
> `Assets/Editor/WorldBuilder.LotCars.cs` and re-run **The Block → Build Lot Cars**. Reverting costs
> nothing because the change wins nothing.
>
> **So: everything below this line is the submission, and it is the whole remaining project.** Six of
> the nine graded artifacts are ✗ against a **1 Oct 2026** deadline. Three rounds of U30b plus U39
> established that the frame is fine and that the remaining performance wins are small or unreachable
> without authored art - **stop optimising**. The video, the kanban board and the zip are worth more
> grade than any millisecond left in the scene. Checklist in the **Submission** § below.
>
...
>
> *"looking good. mark it as done."* The sea is drivable, the sand is sand, and water is not a crime.
> The full account is on the **U36** row. Three things from it are worth carrying, because none of
> them is about the beach:
>
> - ⚠ **The same shader hole may exist elsewhere.** `TheBlock/Beach` rendered dark for weeks because it
>   declared `_SCREEN_SPACE_OCCLUSION` and had **no `DepthNormals` pass**, while this project's URP
>   renderer runs SSAO with DepthNormals as its source - so URP multiplied the sun by garbage
>   occlusion and the sand was lit by sky ambient alone. `TheBlock/Water` is safe **by accident**: it
>   never declares the keyword. **Any hand-written lit shader that does needs all four passes.**
>   `Assets/Police_officer/Materials/Standard(2sides).shader` has never been swept for it. Memory
>   `ssao-depthnormals-kills-the-sun`.
> - **Two new ways to verify without the user.** `render-the-game-view-over-mcp` renders any camera to
>   a PNG and reads exact pixels back while Play is *paused*; `headless-compile-check-via-unity-dotnet`
>   builds both assemblies with the Editor's bundled SDK. Neither replaces a play-test for feel, and
>   both would have saved most of a session here.
> - **U36 owes U30b a frame measurement**, like the six Tier 8 rows: it adds a `DepthNormals` pass on a
>   295 × 600 m mesh and a `WaterEntry` FixedUpdate on every car and bike. Nothing suggests it is
>   expensive; nothing has measured it either.
>
> **Not built, and deliberately:** no splash VFX and no water entry sound. `DamageFx` has the
> runtime-ParticleSystem idiom to copy (`BuildBurst` / `BuildMaterial`) if the user wants them.
>
> **Decided by the user, so do not re-ask:** a sunk vehicle is **lost and respawns home**; staying in a
> submerged one is a **forced bail-out, no drowning and no oxygen bar** (there is no player health
> system to hang one on); and submerging **does** shed the wanted level.
>
...
> ---
>
> **Tier 8's final state, so nobody re-derives it - as of 2026-08-18 EVERY surviving sub-unit is
> user-confirmed:** U35a (ragdolls) ✅, U35b (vehicle damage) ✅, U35c (police helicopter) ✅,
> U35g (the auto shop) ✅, U35h (breakable street props) ✅ **2026-08-18**, U35i (the solid H145) ✅ -
> all six `built - user-confirmed, awaiting U30b`, which is rule 3(a)'s frame measurement on a Player
> that does not exist yet and is the ONLY thing any of them still owes. Dropped by the user: U35d
> (weather), U35e (stunt jumps), U35f (side jobs). U35h shipped both halves: the 233 traffic-light
> poles topple, AND three Sketchfab props the user supplied (bench, bin, cone) stand at the kerb as
> sleeping rigidbodies, the benches on the crowd strips after the re-siting. Section below.
>
...
**The user's ask, and the reason it is a real gap rather than a nice-to-have:** *"we need more way
for our character to make money."* Every money event in the game pays **exactly once per run** -
`CampaignRunner.ReactToEdges` consults `Payouts.Has`, and a replay gets a teardown and nothing else.
Everything that *spends* is repeatable: power-ups at $40-90, paint at $30, fuel, and a bust's fine.
So a cleared campaign leaves a wallet that can only go down, in front of three shops that keep
charging. **This is the faucet.**

**① The asset, modelled from two of the user's photographs.** `tools/build-falafel.py` builds the
whole shop in Blender from scratch - `source-assets/falafel_hapaamonim.blend` →
`Assets/Models/Places/falafel-stand.glb`, **2.1 MB, 41,418 tris, six meshes, zero textures**, the
same recipe as the H145. The corner kiosk, the sign band on the canopy edge (which is where both
photographs put it - *not* on the building), the teal wainscot, the striped awning, outdoor seating,
and an interior you can walk into: counter, twelve-pan vitrine, shawarma spit, tables, stools.

- **Hebrew is the H145's rule, unchanged**: Arial Unicode + hand-reversed strings, because Blender
  does no bidi shaping. New here: a **digit-run fix**, or `מאז 1988` renders as `מאז 8891`.
- **The Coca-Cola wordmark is Snell Roundhand Black**, split out of macOS's `.ttc` with fontTools
  because Blender cannot open a collection. The face is gitignored (Apple's), and the script
  regenerates it from the system font when missing. The two swashes are hand-built tapered ribbons -
  no font has them, and without them the panel reads as red paint.
- **The menu board carries the real menu**, read from 10bis (restaurant 47184): פיתה פלאפל ₪35,
  סביח ₪38, שווארמה ₪68/78, שניצל ₪54.

**② Where it is, and it was measured.** `FalafelSpec.Position` = **(20.25, 0, −96.9)**, yaw −90°, on
the east pavement **directly opposite the pizzeria** at (−20, 0, −100) - the user's placement, in the
20 m gap between `Bench_075` (z −106.93) and `Bench_076` (z −86.93), 6.4 m clear of each. The west
bench row's own gap at z ≈ −94 is the pizzeria's frontage, which is what confirmed the reading.
**Location approved by the user on sight.**

**③ Three marker nodes instead of three C# coordinates.** `fh_vendor`, `fh_talk`, `fh_pin` ship as
empties inside the glb, the idiom `seven-eleven-lot.glb` proved: an empty carries its position
through glTFast untouched, so nothing has to choose between the three handedness rules for a
model-local coordinate. Verified on import - authored Blender `(0.90, 2.15, 0)` arrives as Unity
local `(−0.9, 0, −2.15)`, behind the counter.

**④ The job.** `FalafelRun` is a `MissionBehaviour` that is **deliberately not in
`Campaign.Missions`**, and it is safe from being adopted by construction: `MissionBuilder` orders the
campaign from `campaignText` and drops any mission whose id has no row there. `"falafel"` has none.
**Do not add one.** Everything else is the pizza run's parts - beacons, a guide pin, `F` measured
from the vehicle while driving and the player on foot, `SfxCue.Delivery`. What is new is the loop:
**T at the stand** (from the saddle - the kerb is a measured 2.5 m from the talk point and the radius
is 6), an **escalating streak** (3 drops/150 s/$40 → +1 drop, +20 s, +$15 per round finished back to
back → 6/210 s/$100), and a **loss that resets it to round 1**. Paid straight into `Wallet`, never
through `Payouts`. The 🎒 thermal bag is deliberately NOT consumable here - it doubles a *mission*
payout, and letting a $40 round burn an item bought for the $300 finale is a trap dressed as a
feature.

**⑤ One line in the campaign, and it is the only edit outside new files.** `MissionHud`'s objective,
clock and counter are **latched** setters, unlike the arbitrated prompt, and `CampaignRunner.DriveHud`
rewrites all three every frame. A job writing its own would be clobbered within the same frame. So
`FreeRoamJob.Active` (one writer) stands the campaign down, and the job hands the labels back on
every exit. Registered in `SessionReset` - a flag left set from a torn-down world would leave the
next run with no objective line and no clock and no way to see why.

**⚠ Free-roam police rules apply, and that is a decision.** Crashes still mint stars during a round
and a bust still ends it. `ApplyPoliceRules` was left alone on purpose: its rising edge *clears*
heat, so covering this job with it would turn a falafel counter into a wanted-level launderer.

**Proved in Play before handing it over** (direct method calls, no synthetic keys - see
`synthetic-play-test-decays`): round 1 spawned 3 customers, all 3 delivered, `$0 → $40` **once**,
level → 2; round 2 came up 4 drops / 170 s / $55; `Fail()` dropped it to round 1 and paid nothing
(`$40 → $40`); teardown left **0 stray actors**. Both assemblies compile headlessly.

**⚠ One fault found and fixed, and it is U19e's trap from the other side.** The Asset Store crowd
prefabs arrive at `AnimatorCullingMode.CullCompletely`, so with nobody looking the vendor's Animator
does not run at all and the rig sits in its **bind pose - which this pack binds as a T**. The builder
now sets `AlwaysAnimate`. Verified in Play: arms down, idling. Note an Editor screenshot can never
settle this - Animators do not tick outside Play, so *every* NPC shows a bind pose there, and the
pizzeria cashier only looks right because her Mixamo rig binds in an A-pose.
...
### U35h, 2026-08-17 - breakable street props - BUILT, benches re-sited 2026-08-18, USER-CONFIRMED 2026-08-18

**✅ USER-CONFIRMED 2026-08-18** - *"you can confirm U35H - looking good."* Both halves: the 233
traffic-light poles and the three Sketchfab props (bench, bin, cone), with the benches on the two
crowd strips after the re-siting below. Not `done`, only because rule 3(a)'s frame measurement is on
a Player that does not exist yet - it joins U35a/b/c/g/i in the `awaiting U30b` set. **With it Tier 8
has no unconfirmed unit left.**

**2026-08-18 - the user's first play-test, which produced the re-siting:** *"הפונקציונליות של הספסלים טובה, רק צריך לשנות את המיקום
...
- **TIER 8 IS CLOSED, 2026-08-17.** Four decisions in one message from the user: **U35c and
  ...
  frame measurement.**~~ **Overtaken 2026-08-18: U35h and U35i are both user-confirmed, so all six
  surviving sub-units are confirmed and Tier 8's only remaining debt is U30b's numbers.** No Tier 8
  unit is scheduled.
...
| U36 | The sea is a place, not a wall - sand that reads as sand, and GTA-style water entry | **done - user-confirmed 2026-08-18** | `3a357b3` + `9adcaed` | **USER ASK 2026-08-18, three parts in one message:** *"can we do some effect of sand near the beach? currently we have this black strip. moreover when driving into the water it should not ignite the police... lets do a similar behvaior as we have in real GTA game."* **① THE SAND - SOLVED, and it was not what it looked like.** ✅ **The "black strip" was `TheBlock/Beach` rendering with NO SUN.** The shader declares `_SCREEN_SPACE_OCCLUSION`, this project's URP renderer runs SSAO with **Source = DepthNormals**, and the shader had **no DepthNormals pass** - so it wrote nothing into `_CameraNormalsTexture`, the AO resolve read garbage under its own pixels, and `UniversalFragmentPBR` did `mainLight.color *= aoFactor.directAmbientOcclusion`. The sand kept its albedo and lost the sun, leaving it lit by sky ambient alone. **Measured, not guessed:** albedo `(0.87, 0.78, 0.52)` - correct gold, straight from the config - reaching the screen as `(0.345, 0.376, 0.318)`; the ratio `(0.14, 0.21, 0.37)` is blue-dominant because it IS the sky, and no multiply of a gold albedo can put G above R. `TheBlock/Water` sits beside it looking perfect because it never declares the keyword - two custom shaders on adjacent geometry, one right and one wrong, is most of why this read as a beach problem for weeks. **Fix:** all four passes written out (`ForwardLit`, `DepthNormals`, `ShadowCaster`, `DepthOnly`) sharing one `UnityPerMaterial` CBUFFER in a SubShader `HLSLINCLUDE`; the `UsePass` lines they replace were dragging in URP/Lit's layout, so the shader could never have been SRP Batcher compatible either. Memory `ssao-depthnormals-kills-the-sun`. **A SECOND, REAL bug was found and fixed on the way, and it is worth knowing it was not the reported one:** a seabed-height wet band puts `depth` at 0 across flat dry sand, so `smoothstep(-0.6, 0.5, 0) = 0.54` and the whole beach rendered half-wet with the gold never at full strength. First fixed with a 1.2 m geometric berm - **reverted the same hour**, because raising the terrain buried everything placed on the beach at y 0 (the U27 Beach Dancer measured **1.12 m under**, which is what the user reported as *"האנשים קצת הולכים בתוך החול"*). The tide line is now a DISTANCE from the waterline (`SeaGeometry.TideRun` = 5 m) and the sand is flat. Memory `flat-dry-sand-renders-half-wet`. **Verified by rendering the scene over the MCP and reading pixels back** (memory `render-the-game-view-over-mcp`) - gold sand with a damp band at the water. **② THE POLICE.** Never a water rule. Two invisible 8 m walls stood on the waterline at Unity x 430 - `BuildShoreWall` (600 m) and `BuildWorldEdges`'s Seaward side (1400 m) - so aiming a car at the sea was ramming a wall; `CrashSensor` read the closing speed and `CrimeWatch` compared it to `CrashCrimeSpeed = 6 m/s`, so anything over 21.6 km/h was a star. Both walls moved out to the plate's rim (x 700), opening 270 m of drivable sea, and the sea wall now seats on the seabed rather than on y 0 so a sinking car cannot slide out under it. Memory `an-invisible-wall-reads-as-a-game-rule`. **③ THE WATER.** New `Assets/Scripts/Vehicle/WaterEntry.cs`, attached at runtime by both controllers' `Bind` (never a prefab field - `CrashSensor` was attached to nothing for 14 units that way). Four stages: **Wading** (shallow = drag only, reverse out, no stall - a game where the first centimetre is fatal makes the beach a no-go zone), **Flooding** (sustained depth over `drownAt` for 1.6 s / 0.35 s on a bike cuts the engine, buoyancy bleeds to zero over 7 s), **Sunk** (rests on the bottom, `IEnterable.Respawn` retires it home after 6 s). Floats on `SeaSurface.Height`, never `sea.Level` (memory `vertex-displaced-water-has-no-cpu-height`), four-point buoyancy so a nose-first entry rights itself. A bike ejects its rider the instant it drowns; a car ejects at 80% submersion via the existing `VehicleEnterExit.LeaveVehicleNow`. `Drowned` is its own flag on both controllers, NOT a second writer on `EngineDead` (`VehicleDamage` owns that one). Water drag is a FORCE, not Rigidbody damping - damping is stored state and a mid-Play recompile would cache the water value as the original. `GameAudio` stops the engine loop, `FollowCamera` is held 0.6 m above the surface (`TheBlock/Water` is single-sided, so a camera under it sees straight through to the void the plate was cut away for). **Police, per the user's three answers:** water is never a crime (`CrimeWatch` skips any impact whose sensor is in water - the seabed is a real MeshCollider and landing on it at 60 km/h is a textbook head-on contact), and 4 s in the water WIPES the wanted level (`PoliceTuning.WaterEscapeSeconds`), which is GTA's own rule and the honest one here since nothing the police own can follow you onto water. The water condition folds into `CrimeWatch`'s existing `Heat.Frozen` assignment rather than being written from `WaterEntry` - one flag, one owner. **Compiles clean, both assemblies** (headless, via memory `headless-compile-check-via-unity-dotnet`). **NOT done:** no splash VFX or entry sound - held deliberately so the play-test judges one thing; no drowning (the user chose forced bail-out, and there is no player health system to hang an oxygen bar on). ✅ **USER-CONFIRMED 2026-08-18** - *"looking good. mark it as done."* Verified in the live scene before closing, not assumed: beach mesh flat landward (bounds y −3 → 0, no berm), `_TideRun` 5, `_Debug` 0, the U27 Beach Dancer back at y 0.00 on sand at y 0.00 (it measured 1.12 m under during the berm hour), and the `Sea Wall` at x 700 instead of on the waterline. ⚠ Marked `done` on the user's word rather than `built - awaiting U30b` like the six Tier 8 rows, but it owes the same frame measurement: it adds a `DepthNormals` pass on a 295 × 600 m mesh and a `WaterEntry` FixedUpdate on every car and bike, and neither has been measured |
| U37 + U37a + U37b | פלאפל הפעמונים - a repeatable falafel round, the game's first income faucet | **done - user-confirmed 2026-08-19** | `b13251b` + `fa8331d` + `154954e` | ✅ **USER-CONFIRMED 2026-08-19**, all three at once - *"שניהם נראים טוב, כלומר הכניסה לפלאפל עובדת טוב וגם ההתחלה עם ה T. וגם יש שם מוכר. כל הקטע עם הפלאפל הוא טוב."* So U37a's move indoors, U37b's enterable doorway and the vendor's U38 rebuild are all confirmed in one pass. **The user's ask:** *"we need more way for our character to make money"* - and it is a real gap, not a nice-to-have: every campaign payout is once-per-run through `Payouts`, while power-ups, paint, fuel and fines all keep charging, so a cleared campaign is a wallet that can only go down. **Its own section is above** - the asset, the placement, the job, and the one campaign edit. `tools/build-falafel.py` models the whole shop in Blender from two of the user's photographs (2.1 MB, 41,418 tris, six meshes, ZERO textures) with the Coca-Cola wordmark in Snell Roundhand Black plus two hand-built swashes, and the real 10bis menu on the board. Placed at (20.25, 0, −96.9) yaw −90° **opposite the pizzeria**, in the measured 20 m gap between `Bench_075` and `Bench_076` - **location user-approved on sight**. Three marker empties (`fh_vendor`/`fh_talk`/`fh_pin`) carry the geometry through glTFast rather than three C# coordinates. `FalafelRun` is a `MissionBehaviour` **deliberately NOT in `Campaign.Missions`** and structurally safe from adoption - `MissionBuilder` drops any mission with no `campaignText` row, and `"falafel"` has none, so **do not add one**. T at the stand from the saddle; an escalating streak 3/150 s/$40 → 6/210 s/$100, reset to round 1 by the clock or a bust; paid straight into `Wallet`, never `Payouts`; the 🎒 bag is not consumable here. ⚠ **One line in `CampaignRunner.DriveHud`**: the objective/clock/counter are LATCHED and it rewrites them every frame, so `FreeRoamJob.Active` stands the campaign down while a job owns them (and is cleared in `SessionReset`). ⚠ **Free-roam police rules apply on purpose** - `ApplyPoliceRules` clears heat on its rising edge, so covering this job would make the counter a wanted-level launderer. ⚠ **The vendor arrived T-posed**: the crowd prefabs are `CullCompletely`, so off-screen the Animator never runs and the rig sits in a T bind pose - `AlwaysAnimate` in the builder, U19e's trap from the other side. Proved in Play before hand-off: 3/3 delivered, $0→$40 once, level→2, round 2 = 4 drops/$55, `Fail()` → round 1 paying nothing, 0 stray actors |
