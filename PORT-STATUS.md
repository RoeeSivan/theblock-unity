...
>
> ### 🚩 NEXT ACTION: **play-test U36 - the sea.** Built 2026-08-18 from a three-part user ask, `wip` until the user has driven into the water. Recipe and the two open questions are in the **U36** section directly below. **After that**, the standing next action resumes: **U30b round 2 - the boot hitches** (paragraph below, section further down).
>
> ### U36 - the sea is a place, not a wall
>
> **The user's ask, 2026-08-18:** *"can we do some effect of sand near the beach? currently we have
> this black strip. moreover when driving into the water it should not ignite the police... lets do a
> similar behvaior as we have in real GTA game."* Three parts, all built, none play-tested. The full
> account is on the **U36** row; what a session needs to act is here.
>
> **The play-test recipe, in order:**
>
> 1. Open Unity, let it recompile, **wait for it to finish** (memory `a-menu-item-runs-the-stale-assembly`
>    - a builder run against the old assembly writes the OLD result and says it worked).
> 2. **The Block → Build World.** This is required, not optional: the beach mesh, the sea floor and
>    both boundary walls all move, and none of it exists until the build runs. NavMesh is untouched.
> 3. Play → **Continue** (memory `unity-play-tests-need-continue-invoked` - Play lands on the title
>    menu at `timeScale 0`).
> 4. Drive west to the beach. The sand should read as **gold dry sand with a damp band at the water's
>    edge** - already confirmed by rendering the scene over the MCP and reading the pixels back, so
>    this step is a sanity check rather than an open question.
> 5. Drive in. Expected: splashing through the shallows costs nothing and you can reverse out; keep
>    going and the engine dies, the sound cuts, the car floats a few seconds and goes down; you are
>    thrown out and swim. **The wanted level must not move at any point** - and if you had stars, four
>    seconds in the water clears them.
> 6. Same on the motorcycle - it drowns almost at once and throws you off as it does.
>
> **✅ THE BLACK STRIP IS SOLVED and needed no screenshot in the end** - it was measured. `TheBlock/Beach`
> had no `DepthNormals` pass while SSAO runs with DepthNormals as its source, so URP multiplied the
> sun by garbage occlusion and the sand was lit by sky ambient alone: albedo `(0.87, 0.78, 0.52)`
> reaching the screen as `(0.345, 0.376, 0.318)`. Full account on the U36 row and in memory
> `ssao-depthnormals-kills-the-sun`.
>
> ⚠ **Carry this forward: the same hole may exist elsewhere.** `TheBlock/Water` is safe by accident -
> it never declares `_SCREEN_SPACE_OCCLUSION`, so nothing multiplies its light. **Any hand-written
> lit shader that DOES declare that keyword needs all four passes.**
> `Assets/Police_officer/Materials/Standard(2sides).shader` has not been swept for it.
>
> **Still owed, and it is the user's call:**
>
> - **No splash and no entry sound.** Held back on purpose: memory `feedback-play-test-every-unit`
>   says build one thing and hand over a recipe. `DamageFx` already has the runtime-ParticleSystem
>   idiom to copy (`BuildBurst` / `BuildMaterial`) when this is confirmed.
>
> **Decided by the user in this session, so do not re-ask:** a sunk vehicle is **lost and respawns
> home**; staying in a submerged one is a **forced bail-out, no drowning and no oxygen bar** (there is
> no player health system to hang one on); and submerging **does** shed the wanted level.
>
